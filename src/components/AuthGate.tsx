      ...
      <div className="mx-auto w-full max-w-sm pb-8">
        ...
        <div className="mt-5 rounded-[20px] border border-subtle bg-elevated p-4 shadow-elevated">
          ...
          <form onSubmit={submit} className="mt-4 space-y-3">
            ...
            <label className="block"><span className="text-[10px] uppercase tracking-wider text-content-muted font-semibold">Password</span><div className="relative mt-1.5"><LockKeyhole size={15} className="absolute left-3 top-3.5 text-content-muted" /><input type="password" autoComplete={mode === 'signin' ? 'current-password' : 'new-password'} minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} required className="w-full h-11 rounded-[11px] border border-subtle bg-base pl-9 pr-3 text-[13px] outline-none focus:border-primary" placeholder="At least 6 characters" /></div></label>
