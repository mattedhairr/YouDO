...
export default function SettingsSheet({ open, onClose, onOpenAuth }: Props) {
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in cursor-pointer"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative w-full max-w-md bg-[#0D0B14] border border-white/10 rounded-t-3xl sm:rounded-3xl flex flex-col max-h-[90vh] overflow-hidden shadow-2xl cursor-default"
      >
        {/* ── Header ── */}
        <div
        ...
        >
          ...
          <div>
            <h1 className="text-[15px] font-extrabold text-[#EEE9FC] leading-tight">Settings & App Guide</h1>
            <p className="text-[10px] font-semibold text-[#5F5980] mt-0.5">Account · Data · Features · Guide</p>
          </div>
        </div>
