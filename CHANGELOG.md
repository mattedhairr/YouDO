...
- **Android APK** — versionName **7.5.6**, versionCode **47**. Designed to install over the published **v7.5.3** or unpublished **v7.5.5** candidate with the same package ID and signing key. Apply `cloud_backup_revisions.sql` before installing; the hosted migration succeeded on 2026-09-23. Android install-over and multi-device verification are still pending. This is not a published release.
