# v7.2.0 release review

Release candidate: **v7.2.0 / versionCode 37**, based on the published **v7.1.1 / versionCode 36** release. Package, lockfile, in-app version, and Android versionName agree. The package ID and permanent signing identity are unchanged, so it installs over v7.1.1 without clearing data. A commit or push is not itself a published GitHub Release.

## Verified locally

- 199 automated tests pass, including rolling message expiry, Community context parsing, both Kudos eligibility conditions, confirmation and recovery redirects, offline-mode storage, signed-in sessions, update checks, haptics, reduced-effects preferences, Calendar boundaries, focus persistence, and dialog Back routing. Release-update tests derive their fixtures from the current app version.
...
- 56 isolated PostgreSQL checks pass against the actual community and account-session SQL, including repeat installation, recipient-only rolling expiry, replies, top-three plus completed-bar eligibility, duplicate Kudos prevention, retention cleanup, session ownership, the 24-hour trust boundary, exact revocation, permission denial, and admin account deletion that preserves moderation history. These use disposable local fixtures, not production accounts.
...
- The v7.2.0 room empty state and Board were rechecked in the signed-in preview. A rank-one member below their bar has no Kudos control, and Community/Admin controls arrive in the same committed Board render.
- Startup was reloaded through the signed-in path: the determinate bar advances through session/workspace readiness and remains long enough to visibly reach 100% before the app opens.
...
2. Replace the saved Supabase query with the final committed community setup and rerun it once more. A prior v7.2.0 candidate run succeeded, but release review subsequently corrected expired-message retention. The full query remains idempotent and does not duplicate, promote, or demote accounts. It assigns existing messages their intended `created_at + 24 hours` expiry and removes only ordinary messages already beyond the documented retention window when future cleanup triggers run; open-report evidence is retained.
...
7. Obtain the signed v7.2.0 APK from the release-branch GitHub Actions build. Verify its permanent signing certificate and matching release versions before publishing, then install it over v7.1.1 on a backed-up phone. The Samsung Android 10 keyboard fix remains unchanged from the verified v7.1.1 code.
