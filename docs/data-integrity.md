...
All supplied goal-deletion records are considered before limiting the visible trash list. The pending Batch 5 client also stores a compact, independent deletion ledger for removed goal-node and task IDs through its recoverable device write, backup, import, and account replacement paths. Clearing visible trash does not clear those markers. A marker meeting a live item on another device pauses combination, even if the content matches: the live item might have been deliberately restored. Older clients and older backups do not contain this ledger, so one-sided items still require review.

The Batch 5 cloud-revision upgrade adds a server-incremented revision and compare-and-set write. The client must read a revision before writing and treats a stale revision as a conflict. A legacy client may still write directly; its write also increments the revision. The migration was applied to the hosted project before the candidate client. This prevents a stale whole-backup upload but does not by itself resolve item-level deletion ambiguity.

Hosted checks with the disposable test account saw only its own backup row. Anonymous reads returned no rows and anonymous CAS was denied. A same-content legacy conditional update advanced the revision once; repeating its old timestamp and using the old revision in CAS both returned no rows. Two concurrent, different-content CAS requests accepted exactly one write; the original test backup was then restored and re-read. Two separate browser origins propagated a reversible streak-bar edit. These checks do not prove app-level offline edit, deletion, or restore resolution. The user reported a successful Android install-over without clearing personal data; no conflict experiment was run on that personal workspace.
...
- Complete app-level offline edit and deletion/restore conflict checks. Hosted legacy-write and same-account CAS races pass, but those are not proof that every client conflict screen and recovery path behaves correctly.
